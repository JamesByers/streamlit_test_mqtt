...
st.markdown(f"""
  Last Updated: <span style="color:#1f77b4">{df_temp.iloc[len(df_temp)-1].strftime("%Y-%m-%d %H:%M")} PT</span>
...
temperature_chart = alt.Chart(df2, title= "Temperature").transform_calculate(
    ...
    x=alt.X('Datetime PT:T', axis=alt.Axis(format="%-m/%-d/%-y", tickCount="day", title="Date")),
